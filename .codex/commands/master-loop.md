...
```bash
./.codex/scripts/codex10 loop "[loop directive from user]"
```
...
3. Log:
```bash
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] [master-1] [LOOP_CREATE] \"[loop directive]\"" >> .codex/logs/activity.log
```
...
2. Submit the request via codex10 CLI (coordinator handles routing + persistence)

```bash
./.codex/scripts/codex10 request "[clear description of what the user wants]"
```

**Signal Master-2 immediately:**
```bash
touch .codex/signals/.codex10.handoff-signal
```

**Log:**
```bash
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] [master-1] [REQUEST] \"[description]\"" >> .codex/logs/activity.log
```
...
**Action:**
1. Create fix task via codex10 CLI (URGENT priority)
2. Add lesson to knowledge/mistakes.md
3. Signal Master-3

**Step 1 - Create fix task:**
```bash
./.codex/scripts/codex10 fix "FIX worker-N: [brief description of what needs fixing]"
```

**Signal Master-3:**
```bash
touch .codex/signals/.codex10.fix-signal
```
...
3. Log:
```bash
echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] [master-1] [LOOP_STOP] loop_id=<loop_id>" >> .codex/logs/activity.log
```
...
bash .codex/scripts/signal-wait.sh .codex/signals/.codex10.handoff-signal 20
```
