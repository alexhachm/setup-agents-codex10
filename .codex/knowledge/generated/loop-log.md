...
## Failed Patterns
- Loop 22 iteration 5 (2026-03-16): including backticks in an inline double-quoted `loop-request` command still triggered shell command substitution (`git rebase origin/main`) and stripped the quoted snippet from stored evidence text; keep request text free of backticks or submit via stdin/file payload.
...
## Codebase Gaps
- Fresh dirty-worktree rebase recovery gap (2026-03-16, loop 22 iteration 5): `coordinator/src/merger.js` `tryRebase` runs `git rebase origin/main` directly with no stash preflight/restore, and `.codex/agents/merge-prep.md` Step 1 does the same. Runtime DB evidence from `.codex/state/codex10.db` shows 9 `merge_queue` conflicts with `cannot rebase: You have unstaged changes ... Please commit or stash them` (examples: merge ids #55 and #47), causing repeated remediation churn. Submitted `req-ab095881`.
...
## Iteration Updates (Loop 20, Iteration 1 - 2026-03-16)

### Successful Patterns
- Loop 20 iteration 1: an isolated temp-DB repro (without touching live state) provided high-confidence allocator-scope evidence by showing claimed worker state before/after a single watchdog tick.
- Loop 20 iteration 1: focusing on one ownership-lifecycle contradiction (`claimWorker` persistence vs watchdog stale-claim age source) kept research concrete and actionable.

### Failed Patterns
- Loop 20 iteration 1: `loop-request` submission was suppressed by `quality_gate` with `missing concrete file path signal (WHERE)` despite file references being present; avoid markdown/backtick formatting in WHERE lines and use plain absolute/relative file tokens only.

### Codebase Gaps
- `coordinator/src/watchdog.js` `releaseStaleClaimsCheck` computes claim age from `worker.last_heartbeat || worker.created_at`, while `coordinator/src/db.js` `claimWorker` persists only `claimed_by` and no claim timestamp. Repro on 2026-03-16: worker heartbeat set to 10 minutes old, `claimWorker(1,'master-2')` succeeds, then one `watchdog.tick()` clears `claimed_by` back to `null` immediately, releasing a fresh reservation.

### False Positives
- None observed in this iteration.

## Iteration Updates (Loop 20, Iteration 2 - 2026-03-16)

### Successful Patterns
- Loop 20 iteration 2: running overlap/parity gates before submission (`status`, `request-history`, `git show origin/main`) prevented duplicate allocator requests when local branch behavior diverged from upstream.
- Loop 20 iteration 2: isolated temp-DB repros remained useful for confidence checks, but were only treated as submission-grade after origin/main parity validation.

### Failed Patterns
- Loop 20 iteration 2: phase-2 `loop-requests` had no loop-owned request outcomes (`No requests for this loop`), so there was no fresh completed/failed pattern signal to reuse this pass.

### Codebase Gaps
- Local branch `coordinator/src/cli-server.js` still shows claimed-worker assignment bypass in `assign-task`, but origin/main already contains `worker_claimed` rejection; treat as branch drift unless parity regresses.
- Local branch `coordinator/src/watchdog.js` still has immediate non-conflict merge-failure finalization with sparse allocator payload, but origin/main now includes remediation-task/grace checks plus per-merge context payloads.

### False Positives
- Did not submit a claimed-worker assignment request after reproducing it locally because `origin/main` already has the guard and the issue overlaps previously completed allocator ownership fixes.
- Did not submit the non-conflict `merge_failed` lifecycle candidate after repro because `origin/main` already includes the remediation grace/window logic and richer `merge_failed` mail payloads.

## Iteration Updates (Loop 20, Iteration 3 - 2026-03-16)

### Successful Patterns
- Loop 20 iteration 3: status + completion preflight (`status`, `check-completion req-6a20de1d`, `ready-tasks`) before source review prevented duplicate submissions while allocator remediation requests remain integrating.
- Loop 20 iteration 3: combining local source inspection with `origin/main` parity checks (`cli-server.js` loop handlers) filtered branch-drift regressions and supported a high-confidence no-submit decision.

### Failed Patterns
- Loop 20 iteration 3: phase-2 `loop-requests` again returned `No requests for this loop`, so there were no loop-owned completed/failed outcomes to harvest for request-shape tuning.

### Codebase Gaps
- `coordinator/src/watchdog.js` `monitorLoops()` still skips active loops missing `tmux_window` (`if (!loop.tmux_window) continue`), which can leave stale-heartbeat recovery blind on non-tmux launch paths; this is adjacent to active stale-loop remediation (`req-0eb53b12`), so no overlapping request was submitted this iteration.

### False Positives
- Local `coordinator/src/cli-server.js` still shows ungated `loop-checkpoint`/`loop-heartbeat` updates, but `origin/main` already enforces active-status guards; treated as branch drift, not a net-new allocator submission.

## Iteration Updates (Loop 20, Iteration 4 - 2026-03-16)

### Successful Patterns
- Loop 20 iteration 4: checkpoint-guided preflight (`status`, `check-completion req-6a20de1d`, `check-completion req-0eb53b12`) plus `loop-findings` overlap scan prevented duplicate stale-loop submissions while active remediation requests remain integrating.
- Loop 20 iteration 4: validating allocator prompt mirrors against launch/setup wiring (`templates/commands/allocate-loop.md`, `.codex/commands-codex10/allocate-loop.md`, `setup.sh`, `scripts/launch-agent.sh`) and then cross-checking `origin/main` avoided re-submitting a known merge-conflicted scope.

### Failed Patterns
- Loop 20 iteration 4: `loop-requests` returned `No requests for this loop`, so there were no loop-owned completed/failed outcomes to mine for new request-shape adjustments.

### Codebase Gaps
- Local branch still has stale allocator mirror text in `templates/commands/allocate-loop.md` (deprecated `.codex10.*` signal waits and `request_ready_to_merge` wording) while setup force-copies templates into `.codex/commands-codex10`; in this iteration it was treated as branch-local drift because `origin/main` already carries mailbox-blocking guidance.

### False Positives
- Did not submit allocator prompt-mirror follow-up: the same scope is already represented by failed `req-1c0d8c67` (merge conflict path), and `origin/main` parity check shows the template guidance already updated upstream.

## Iteration Updates (Loop 20, Iteration 5 - 2026-03-16)

### Successful Patterns
- Loop 20 iteration 5: combining checkpoint-directed overlap review (`loop-requests`, `status`, `request-history`) with direct source/runtime contradiction checks (`scripts/loop-sentinel.sh`, `.codex/scripts/loop-sentinel.sh`, `coordinator/src/watchdog.js`, and live `activity_log`) produced a distinct high-confidence allocator-loop submission (`req-81a6bd11`) without duplicating active stale-loop recovery scope.
- Loop 20 iteration 5: explicit WHAT/WHERE/WHY/EVIDENCE wording that separated heartbeat-cadence hardening from existing watchdog-recovery work (`req-0eb53b12`) passed the loop quality gate on first attempt.

### Failed Patterns
- Loop 20 iteration 5: phase-2 outcome review returned `No requests for this loop`, so no completed/failed loop-owned execution outcomes were available to calibrate from this pass.

### Codebase Gaps
- `scripts/loop-sentinel.sh` and `.codex/scripts/loop-sentinel.sh` call `loop-heartbeat` once per iteration while healthy cadence uses long backoff sleeps; `coordinator/src/watchdog.js` flags `loop_heartbeat_stale` at `>300s`, creating false stale-loop telemetry for live loops.
- Runtime evidence in this iteration showed `loop_heartbeat_stale` for loop 20 while allocator continued assigning tasks, indicating heartbeat cadence and stale threshold are currently misaligned.

### False Positives
- Did not file a duplicate stale-loop recovery request because active `req-0eb53b12` already targets watchdog restart behavior; this iteration focused on heartbeat-cadence hardening as the non-overlapping root-cause gap.

## Iteration Updates (Loop 20, Iteration 6 - 2026-03-16)

### Successful Patterns
- Loop 20 iteration 6: checkpoint-targeted preflight (`status`, `check-completion`, `request-history`) before drafting new scope kept overlap analysis grounded and avoided resubmitting active tmux stale-heartbeat work.
- Loop 20 iteration 6: pairing `origin/main` parity checks with local source inspection (`coordinator/src/index.js`, `coordinator/src/watchdog.js`) isolated a distinct non-tmux lifecycle gap rather than branch-only drift.

### Failed Patterns
- Loop 20 iteration 6: `loop-request` submission was suppressed by throughput cooldown (`retry_after=14s`) despite passing quality criteria; avoid submitting before `loop_request_min_interval_sec` elapses.

### Codebase Gaps
- `coordinator/src/index.js` non-tmux loop launch path (onLoopCreated else branch) starts `scripts/loop-sentinel.sh` detached without persisting `tmux_window`, while `coordinator/src/watchdog.js` `monitorLoops()` immediately skips `active` loops missing `tmux_window`; this leaves non-tmux loops outside stale-heartbeat monitoring/recovery and can stall allocator liveness on Windows/no-tmux runtime.
- `coordinator/tests/watchdog.test.js` currently has no regression coverage for active loops with `tmux_window` null, so this non-tmux monitoring gap can regress silently.

### False Positives
- No overlap found with active `req-0eb53b12`; that request targets stale loops with existing tmux panes, while this iteration's candidate targets loops skipped entirely when `tmux_window` is null.

## Iteration Updates (Loop 21, Iteration 1 - 2026-03-16)

### Successful Patterns
- Loop 21 iteration 1: direct command-level repro plus origin/main parity validation isolated a net-new Master-2 triage correctness defect and supported a high-confidence submission (`req-b64c7fec`) without overlapping active loop-lifecycle work.

### Failed Patterns
- Loop 21 iteration 1: embedding backticked shell commands inside `loop-request` text triggered command substitution in bash; evidence strings should use plain quotes or escaped backticks to avoid mangled descriptions.

### Codebase Gaps
- `coordinator/src/cli-server.js` `triage` updates/logs without verifying request existence; `./.codex/scripts/codex10 triage req-does-not-exist 3 "probe"` returns success and logs a triage event for a non-existent request, leaving actual request state unchanged.

### False Positives
- Local inbox filter mismatch (`mac10 inbox --type/--request-id`) is already fixed on `origin/main`; treat as branch drift and do not re-submit.

## Iteration Updates (Loop 20, Iteration 7 - 2026-03-16)

### Successful Patterns
- Loop 20 iteration 7: combining checkpoint-directed overlap checks (`status`, `check-completion`, `request-history`) with direct source parity validation against `origin/main` prevented duplicate stale-loop scope and confirmed a net-new non-tmux monitoring gap.
- Loop 20 iteration 7: an isolated temp-DB watchdog repro (active loop with `tmux_window=null` and stale heartbeat) produced concrete behavior evidence (`watchdog.tick()` emitted no loop monitoring actions), which passed the quality gate and landed as `req-8c1be75e`.

### Failed Patterns
- None observed in this iteration.

### Codebase Gaps
- `coordinator/src/watchdog.js` `monitorLoops()` still short-circuits with `if (!loop.tmux_window) continue`, so active non-tmux loops are excluded from stale-heartbeat monitoring and recovery.
- `coordinator/src/index.js` non-tmux loop launch path creates detached loop sentinels without setting `tmux_window`, making the watchdog skip path reachable in live runtime state (for example loop 21 at 2026-03-16 20:27).
- `coordinator/tests/watchdog.test.js` lacks regression coverage for non-tmux (`tmux_window=null`) active loop monitoring/recovery behavior.

### False Positives
- Did not resubmit tmux stale-recovery (`req-0eb53b12`) or heartbeat-cadence (`req-81a6bd11`) scopes; evidence showed the new defect is a separate non-tmux branch gap.

## Iteration Updates (Loop 21, Iteration 2 - 2026-03-16)

### Successful Patterns
- Loop 21 iteration 2: rechecking loop-owned lifecycle first (`loop-requests`, `check-completion`, `request-history`) showed `req-b64c7fec` progressed to integrating with `1/1` tasks complete, which prevented duplicate triage-silent-success scope.
- Loop 21 iteration 2: combining direct CLI repro commands with source inspection (`coordinator/bin/mac10`, `coordinator/src/cli-server.js`) produced high-confidence architect-facing evidence around command success semantics.

### Failed Patterns
- Loop 21 iteration 2: `loop-request` submission was suppressed by throughput cooldown (`retry_after=52s`); avoid submitting before `loop_request_min_interval_sec` elapses.

### Codebase Gaps
- `coordinator/bin/mac10` currently prints command-level errors for `create-task` but still exits with status `0`, which can mislead shell automation.
- `coordinator/src/cli-server.js` `tier1-complete` path updates/logs/sends completion mail without verifying that `request_id` exists, allowing false completion acknowledgements for unknown requests.

### False Positives
- Unknown-request `create-task` is not a DB integrity fail-open: SQLite foreign key enforcement correctly rejects the insert; the exposed issue is CLI exit semantics (plus `tier1-complete` existence validation).

## Iteration Updates (Loop 20, Iteration 8 - 2026-03-16)

### Successful Patterns
- Loop 20 iteration 8: checkpoint-directed overlap preflight (`loop-requests`, `status`, `check-completion`, `request-history`) before new research prevented duplicate submission against active watchdog requests (`req-8c1be75e`, `req-81a6bd11`).
- Loop 20 iteration 8: direct source contradiction checks across prompt mirrors plus setup wiring (`templates/commands/allocate-loop.md`, `.codex/commands-codex10/allocate-loop.md`, `.codex/commands/allocate-loop.md`, `setup.sh`) isolated a concrete re-seeding risk rather than speculative runtime drift.

### Failed Patterns
- Loop 20 iteration 8: no loop-owned completed/failed outcomes were available from `loop-requests` (both loop requests remain `decomposed`), so outcome-learning had to rely on broader shared findings.
- Loop 20 iteration 8: submission attempt failed operationally because the loop transitioned to stopped during execution (`loop-request` returned "Loop is stopped, not active"; `loop-heartbeat` exited with code 2).

### Codebase Gaps
- `templates/commands/allocate-loop.md` still contains stale allocator guidance (`request_ready_to_merge` and `.codex10.task-signal/.codex10.fix-signal/.codex10.completion-signal` waits), while current codex10 allocator canon in `.codex/commands/allocate-loop.md` and `.codex/commands-codex10/allocate-loop.md` uses mailbox-blocking wake flow (`inbox allocator --block --timeout`) and deprecates those signal files.
- `setup.sh` always refresh-copies `templates/commands/*.md` into `.codex/commands-codex10` (lines 155-157), so stale template text can overwrite corrected allocator command prompts on fresh setup/re-setup.
- `templates/docs/master-3-role.md` and `.codex/docs/master-3-role.md` still instruct watching deprecated `.codex10.*` signal files, conflicting with current mailbox-event producers (`tasks_available`, `tasks_ready`, `task_completed`, `task_failed`, `functional_conflict`, `merge_failed`) in coordinator runtime.

### False Positives
- Did not re-open non-tmux watchdog monitoring scope: active request `req-8c1be75e` already targets `monitorLoops` non-tmux skip behavior.
- Did not re-open heartbeat-cadence stale-loop scope: active request `req-81a6bd11` already targets sentinel heartbeat cadence vs stale threshold alignment.

## Iteration Updates (Loop 22, Iteration 1 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 1: combining direct CLI repro commands with source inspection in `coordinator/bin/mac10` isolated a concrete allocator-path reliability defect (command errors returning exit code 0) with high-confidence evidence across multiple commands.

### Failed Patterns
- Loop 22 iteration 1: `loop-request` was suppressed by `quality_gate` (`missing concrete file path signal (WHERE)` and `missing concrete change verb (WHAT)`) when the submission used non-allowlisted WHAT wording and only a non-extension path token; next submission should include allowlisted verbs such as "Fix" and at least one explicit path token like `coordinator/tests/cli.test.js`.

### Codebase Gaps
- `coordinator/bin/mac10` command handlers log `result.error` to stderr but generally do not call `process.exit(1)`, so failing operations can return exit code 0.
- Reproduced on 2026-03-16 in this workspace: `./.codex/scripts/codex10 assign-task 999999 1` -> `task_not_ready` with exit 0; `printf '{"request_id":"req-does-not-exist","subject":"probe","description":"probe","tier":2}' | ./.codex/scripts/codex10 create-task -` -> `FOREIGN KEY constraint failed` with exit 0; `./.codex/scripts/codex10 integrate req-does-not-exist` -> `Not all tasks completed` with exit 0.

### False Positives
- None observed in this iteration.

## Iteration Updates (Loop 22, Iteration 2 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 2: confirming runtime path parity first (`./.codex/scripts/codex10` -> root `coordinator/bin/mac10`) plus direct failing-command exit-code capture produced a high-confidence allocator-path reliability request that passed quality gate (`req-034d2f1d`).
- Loop 22 iteration 2: using allowlisted WHAT wording (`Fix`) and explicit file tokens (`coordinator/bin/mac10`, `coordinator/tests/cli.test.js`) avoided the prior quality-gate suppression for missing WHAT/WHERE signals.

### Failed Patterns
- Phase-2 outcome review again had no loop-owned completed/failed requests (`loop-requests` returned no rows before submission), so pattern tuning had to rely on shared findings and direct repro evidence.

### Codebase Gaps
- Root runtime CLI in `coordinator/bin/mac10` still returns exit code 0 for many coordinator RPC failures because handlers print `result.error` without `process.exit(1)` (for example around `create-task`, `assign-task`, and `integrate`).
- `coordinator/tests/cli.test.js` lacks regression assertions for CLI non-zero exits on failing commands, allowing shell-automation false-success regressions to persist.

### False Positives
- Local worktree copy of `coordinator/bin/mac10` showed unrelated syntax drift in `coordinator/src/db.js`; request evidence was correctly anchored to the root runtime binary used by `.codex/scripts/codex10`, avoiding a branch-drift false positive.

## Iteration Updates (Loop 22, Iteration 3 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 3: running overlap preflight (`status`, `request-history`, `check-completion`) before drafting prevented a duplicate submission while active reconciliation work (`req-b78f6d2b`) is still decomposed.
- Loop 22 iteration 3: validating against live DB rows (`requests`, `tasks`, `merge_queue`) plus source inspection in `coordinator/src/db.js` and `coordinator/src/cli-server.js` produced high-confidence evidence for a no-submit decision instead of speculative re-requests.

### Failed Patterns
- Phase-2 outcome review still had no loop-owned completed/failed outcomes (`req-034d2f1d` remains `decomposed`), so this pass could not calibrate request wording from fresh execution results.

### Codebase Gaps
- `coordinator/src/db.js` `checkRequestCompletion` reports `all_completed=true` when all tasks are completed even if the request itself is already terminal `failed` from merge conflicts; current runtime shows `req-8c1be75e` and `req-0eb53b12` as `failed` in `status`, while `check-completion` returns `1/1 completed, 0 failed — ALL DONE`.
- State divergence is visible in live data: request rows for `req-8c1be75e`/`req-0eb53b12` are `failed` with merge-conflict results, but their only task rows are `completed`, demonstrating request/task/merge lifecycle drift that needs reconciliation.

### False Positives
- Did not submit a new loop request for the above lifecycle drift because it overlaps active decomposed request `req-b78f6d2b` (state-machine invariants/reconciliation scope) and would duplicate in-flight work.

## Iteration Updates (Loop 22, Iteration 4 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 4: performing the iteration-3 memory recall against checkpoint EXPLORED scope before deep research helped keep investigation focused on remaining areas (quality-gate parser edges, merger rebase fallback, allocator conflict mail flow).
- Loop 22 iteration 4: overlap-first runtime checks (`status`, `check-completion req-b78f6d2b`, task/merge inspection) confirmed the broad orchestration remediation request is still in progress (`2/5` tasks completed), avoiding a duplicate lifecycle submission.

### Failed Patterns
- Phase-2 outcome review still had no loop-owned completed/failed requests (`req-034d2f1d` remains `decomposed`), so request-shape calibration again relied on shared findings plus fresh runtime evidence.

### Codebase Gaps
- `templates/commands/loop-agent.md` is stale relative to `.codex/commands/loop-agent.md` and `.codex/commands-codex10/loop-agent.md`: it still references `./.claude/scripts/codex10`, allows `1-3` submissions, and omits `EVIDENCE`/`CONFIDENCE` requirements. Because setup syncs templates into runtime command mirrors, this can re-seed weaker loop-agent guidance on fresh installs.
- `coordinator/src/merger.js` still logs/handles tier-3 failures as `merge_failed` events, while current allocator guidance treats `merge_failed` as deprecated/ignore; this mismatch is observable in runtime logs (`coordinator: merge_failed` at 2026-03-16 21:06:51) and warrants follow-up once the active orchestration hardening request settles.

### False Positives
- Did not submit the `merge_failed` event mismatch this iteration because it likely overlaps in-flight broad hardening scope under `req-b78f6d2b` (task 66 already covers merger preflight/retry/circuit-breaker themes).

## Iteration Updates (Loop 22, Iteration 6 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 6: checkpoint-directed preflight (`loop-requests --json`, `check-completion`, `status`) quickly confirmed both loop-owned requests were still active (`req-ab095881` pending, `req-034d2f1d` decomposed), which prevented overlap with in-flight code fixes.
- Loop 22 iteration 6: pairing allocator prompt inspection with runtime source anchors surfaced a concrete contradiction on `merge_failed` handling (`.codex/commands-codex10/allocate-loop.md` lines 63-66 vs 196-199, plus live emitters in `coordinator/src/merger.js` and `coordinator/src/watchdog.js`), producing a high-confidence candidate gap.

### Failed Patterns
- Loop 22 iteration 6: passing a `loop-request` payload with unescaped backticks/file tokens through double quotes caused shell command substitution before CLI parsing, and the submission was suppressed by quality gate (`missing concrete file path signal (WHERE)`); future submissions should avoid shell-interpreted formatting (plain text or single-quoted payload) to preserve WHAT/WHERE tokens.

### Codebase Gaps
- Active allocator codex10 prompt in `.codex/commands-codex10/allocate-loop.md` is internally inconsistent: it lists `merge_failed` as a remediation-triggering mailbox event but later marks `merge_failed` deprecated/ignore, while coordinator runtime still emits `merge_failed` from merger and watchdog paths.

### False Positives
- None observed in this iteration.

## Iteration Updates (Loop 22, Iteration 7 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 7: re-validating runtime sources before submission decisions (`coordinator/src/merger.js` + `coordinator/src/watchdog.js`) confirmed `merge_failed` is still actively emitted, preventing a docs-only assumption.
- Loop 22 iteration 7: checking active-request overlap first (`status` + `request-history req-b78f6d2b`) prevented a likely duplicate submission while broad orchestration hardening remains decomposed/in flight.

### Failed Patterns
- Phase-2 outcome review again had no new loop-owned completed/failed outcomes (`req-ab095881` pending, `req-034d2f1d` decomposed), so wording calibration from fresh execution results was unavailable.

### Codebase Gaps
- Allocator guidance is still contradictory on `merge_failed`: `.codex/commands-codex10/allocate-loop.md` line 65 lists `merge_failed` as remediation-triggering, lines 196-198 mark it deprecated/ignore, `templates/commands/allocate-loop.md` still treats it as active remediation, and runtime emitters in `coordinator/src/merger.js` (`escalateToAllocator` non-functional path) plus `coordinator/src/watchdog.js` (stale integration failure paths) continue to send allocator `merge_failed` mail.

### False Positives
- Did not submit a new loop request for the `merge_failed` guidance mismatch in this iteration because overlap risk remains high with active complex request `req-b78f6d2b` (merger retry/escalation/state-machine hardening scope).

## Iteration Updates (Loop 22, Iteration 8 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 8: checkpoint-directed overlap preflight (`loop-requests`, `status`, `check-completion`, `request-history`) confirmed loop-owned requests were still in-flight and avoided duplicate scope.
- Loop 22 iteration 8: pairing prompt-file line contradictions with runtime evidence (`coordinator/src/merger.js`, `coordinator/src/watchdog.js`, and live `mail` rows) produced a high-confidence allocator-guidance request that was accepted as `req-07dc7660`.
- Loop 22 iteration 8: validating setup reseed wiring (`setup.sh` force-copy paths) before submission confirmed the prompt drift is operational, not doc-only.

### Failed Patterns
- No new completed/failed loop-owned request outcomes were available this iteration (`req-ab095881` still `pending`, `req-034d2f1d` still `decomposed`), so wording calibration from fresh execution results was limited.

### Codebase Gaps
- Allocator guidance remains inconsistent across mirrors: `.codex/commands-codex10/allocate-loop.md` simultaneously says `merge_failed` should trigger remediation and later says it is deprecated/ignore.
- `.codex/commands/allocate-loop.md` and `templates/commands/allocate-loop.md` still include stale `.claude` command paths and merge-failure instructions that diverge from codex10 runtime intent.
- `.codex/docs/master-3-role.md` and `templates/docs/master-3-role.md` still instruct watching deprecated `.codex10.*` signal files.
- `setup.sh` force-refresh copies can re-seed stale allocator command/docs content into runtime files on setup reruns.

### False Positives
- Did not submit a duplicate dirty-worktree merge request because `req-ab095881` is already active and directly targets that conflict loop.
- Did not submit another CLI exit-code request because `req-034d2f1d` is already active for that scope.

## Iteration Updates (Loop 22, Iteration 9 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 9: checkpoint-directed overlap preflight (`loop-requests`, `status`, `check-completion`, `request-history`) confirmed loop-owned requests `req-ab095881`, `req-034d2f1d`, and `req-07dc7660` remain active, preventing duplicate submissions.
- Loop 22 iteration 9: validating candidate gaps against both source and live DB/runtime state prevented a speculative request and kept this pass at no-submit quality.

### Failed Patterns
- Loop 22 iteration 9: no loop-owned requests reached `completed` or `failed`, so outcome-shape calibration from fresh execution results was unavailable.

### Codebase Gaps
- `coordinator/src/allocator.js` emits allocator mail `research_batches_available` and queries research queue tables, while allocator guidance files (`.codex/commands-codex10/allocate-loop.md`, `.codex/commands/allocate-loop.md`, `templates/commands/allocate-loop.md`, `.codex/docs/master-3-role.md`) currently contain no explicit handling path for this research-batch event family.

### False Positives
- Initial DB checks against the worktree-local `.codex/state/codex10.db` produced schema mismatch signals (`no such table: research_batches`) that do not prove a runtime allocator defect by themselves.
- Root runtime DB (`/mnt/c/Users/Owner/Desktop/setup-agents-codex10/.codex/state/codex10.db`) also lacks research tables in this live session, so the research-batch handling candidate was not submitted due insufficient runtime confirmation.

## Iteration Updates (Loop 22, Iteration 10 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 10: running overlap preflight (`loop-requests`, `status`, `check-completion`, `request-history`) before deep research prevented duplicate submissions while loop-owned requests remained active (`req-ab095881`, `req-07dc7660`, `req-034d2f1d`).
- Loop 22 iteration 10: honoring the every-3rd-iteration memory check by relying on checkpoint `EXPLORED`/`REMAINING` fields kept this pass focused on unresolved areas instead of re-scanning already covered allocator/merger surfaces.

### Failed Patterns
- Loop 22 iteration 10: no loop-owned requests reached `completed` or `failed`, so outcome-based wording calibration remains unavailable for this pass.

### Codebase Gaps
- No new non-overlapping, high-confidence allocator-loop defect met the submission gate in this iteration.

### False Positives
- Suspected backward-compatibility risk from `coordinator/src/allocator.js` research queue queries against DBs missing `research_*` tables was not submitted: source verification in `coordinator/src/db.js` shows `init()` always calls `ensureResearchBatchingSchema()` after schema load, so upgraded runtimes create required tables before allocator ticks.
- Runtime/root observations showing absent `research_*` tables and older allocator behavior were treated as environment/version drift (running coordinator code not matching this worktree), not proof of a new request-worthy defect in current source.

## Iteration Updates (Loop 22, Iteration 11 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 11: checkpoint-directed overlap preflight (`loop-requests`, `status`, `check-completion`, and request-history for `req-ab095881`, `req-07dc7660`, `req-034d2f1d`) confirmed all loop-owned requests remain active, preventing duplicate submissions.
- Loop 22 iteration 11: validating candidate allocator gaps against active request scope (`req-59bfc6fb`, `req-b78f6d2b`) plus runtime DB evidence (`mail`/`activity_log` research_batch queries) filtered speculative submissions and preserved high-confidence gating.

### Failed Patterns
- Loop 22 iteration 11: no loop-owned requests reached `completed` or `failed`, so outcome-based wording calibration remains unavailable this pass.

### Codebase Gaps
- Allocator runtime includes research orchestration surfaces (`coordinator/src/allocator.js` `research_batches_available`; `coordinator/src/cli-server.js` `research_batch_dispatched` / `research_batch_collected`; `coordinator/src/watchdog.js` `research_batch_timeout`) while allocator guidance files currently contain no `research_batch*` handling instructions.
- Current runtime DB snapshot did not show live allocator `research_batch*` mail/activity rows, so the gap remains a candidate pending stronger runtime contradiction.

### False Positives
- Potential request on allocator `research_batch*` guidance mismatch was held this iteration due overlap risk with active deep-research orchestration request `req-59bfc6fb` and lack of current live `research_batch*` mailbox evidence.

## Iteration Updates (Loop 22, Iteration 12 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 12: checkpoint-directed overlap preflight (`loop-requests`, `status`, and `check-completion` for `req-ab095881`, `req-07dc7660`, `req-034d2f1d`) confirmed all loop-owned requests are still non-terminal, preventing duplicate submissions.
- Loop 22 iteration 12: combining active-request history checks (`req-59bfc6fb`, `req-b78f6d2b`, `req-07dc7660`) with direct source scans in `coordinator/src/allocator.js`, `coordinator/src/cli-server.js`, and `coordinator/src/watchdog.js` maintained non-overlap discipline before considering new request scope.

### Failed Patterns
- Loop 22 iteration 12: no loop-owned requests reached `completed` or `failed`, so outcome-based wording calibration remains unavailable.

### Codebase Gaps
- Allocator runtime still contains `research_batch*` mailbox paths (`research_batches_available`, `research_batch_dispatched`, `research_batch_collected`, `research_batch_timeout`) while allocator guidance remains primarily focused on task/merge events; this remains a candidate gap pending stronger runtime activity evidence outside active deep-research work.

### False Positives
- Did not submit a new allocator `research_batch*` guidance request this iteration because live runtime DB checks returned zero `research_batch%` mail/activity rows and scope overlap risk remains high with active deep-research request `req-59bfc6fb`.
- Loop 22 iteration 13 (2026-03-16): phase-2 review found no loop-scoped completed/failed outcomes (`req-07dc7660` pending, `req-ab095881` pending, `req-034d2f1d` decomposed), so submission quality must continue to rely on fresh status/check-completion/source contradiction evidence instead of outcome-derived pattern shifts.

## Iteration Updates (Loop 22, Iteration 13 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 13: checkpoint-directed preflight (`status`, `check-completion` for `req-ab095881`/`req-07dc7660`/`req-034d2f1d`, and request-history for `req-59bfc6fb`/`req-b78f6d2b`) confirmed active overlap boundaries before considering new scope.
- Loop 22 iteration 13: runtime DB validation via Node + `better-sqlite3` (fallback when `sqlite3` CLI is unavailable) preserved evidence quality and avoided speculative allocator `research_batch*` submissions.

### Failed Patterns
- Loop 22 iteration 13: no loop-owned requests reached `completed` or `failed`, so there is still no new outcome-driven calibration signal this pass.

### Codebase Gaps
- `req-59bfc6fb` remains decomposed with unfinished tasks (`#8` ready, `#9` pending, `#81` assigned) but live DB still shows zero `research_batch%` mail/activity rows, leaving allocator `research_batch*` handling as an unproven runtime gap.

### False Positives
- Did not submit a new request this iteration because any allocator/merge conflict request would overlap active `req-ab095881` and `req-59bfc6fb` scopes without new contradictory runtime evidence.

## Iteration Updates (Loop 22, Iteration 14 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 14: preflight overlap checks (`loop-requests`, `status`, `check-completion`, `request-history`) again confirmed loop-owned requests are still active (`req-ab095881` pending, `req-07dc7660` pending, `req-034d2f1d` decomposed), preventing duplicate submissions.
- Loop 22 iteration 14: using the active runtime DB path `.codex/state/codex10.db` for evidence checks (after ruling out the missing `coordination.db` path) kept runtime validation accurate.

### Failed Patterns
- Loop 22 iteration 14: no loop-owned requests reached `completed` or `failed`, so outcome-based calibration remains unavailable.

### Codebase Gaps
- Research-batch orchestration code paths remain present across allocator/runtime files (`coordinator/src/allocator.js`, `coordinator/src/cli-server.js`, `coordinator/src/watchdog.js`) while allocator guidance still lacks explicit `research_batch*` handling steps; however, this remains unproven as an immediate runtime defect.

### False Positives
- Did not submit a request on `research_batch*` guidance parity this iteration because live runtime DB queries still returned zero `research_batch%` mail and activity rows, and scope overlap remains high with active deep-research request `req-59bfc6fb`.
- Did not submit merge-conflict/remediation requests because active pending scope already covers that area (`req-ab095881`, `req-07dc7660`), and no new contradiction emerged beyond existing evidence.

## Iteration Updates (Loop 22, Iteration 15 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 15: overlap preflight (`loop-requests`, `status`, `check-completion`, and `request-history`) again prevented duplicate submissions by confirming loop-owned requests remain active (`req-ab095881`, `req-07dc7660`, `req-034d2f1d`) and deep-research remediation is still in progress (`req-59bfc6fb`, `req-b78f6d2b`).
- Loop 22 iteration 15: runtime evidence fallback to Node + `better-sqlite3` remained effective when `sqlite3` CLI was unavailable, allowing direct checks of live `mail`/`activity_log` rows.

### Failed Patterns
- Loop 22 iteration 15: no loop-owned requests reached `completed` or `failed`, so there is still no new outcome-based wording calibration signal.

### Codebase Gaps
- Runtime continues to emit merge-failure pressure (`merge_queue` conflict backlog and `merge_failed` pathways in `merger.js`/`watchdog.js`), but active pending requests already cover dirty-rebase protection and allocator guidance parity.

### False Positives
- Did not submit a new request this iteration: candidate merge/remediation issues overlapped active pending scope (`req-ab095881`, `req-07dc7660`, `req-b78f6d2b`) and no distinct non-overlapping contradiction was found.

## Iteration Updates (Loop 22, Iteration 16 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 16: overlap preflight (`loop-requests`, `status`, `check-completion`, `request-history`) again prevented duplicate submissions against active scopes (`req-ab095881`, `req-07dc7660`, `req-034d2f1d`).
- Loop 22 iteration 16: isolated temp-DB runtime repros against the main project tree (`/mnt/c/Users/Owner/Desktop/setup-agents-codex10`) produced concrete evidence without mutating live coordinator state.

### Failed Patterns
- Loop 22 iteration 16: `loop-request` submission was suppressed by quality gate (`missing concrete change verb (WHAT)`) despite detailed evidence; future WHAT clauses should start with an explicit imperative verb such as `Fix`/`Add`/`Update` at sentence start.

### Codebase Gaps
- `coordinator/src/watchdog.js` `recoverStaleIntegrations` still marks requests `completed` in the `allMerged` branch using merge rows alone, without validating sibling task success; isolated repro showed `integrating -> completed` even while `checkRequestCompletion` reported `total=2, completed=1, failed=1`.

### False Positives
- Local worktree `coordinator/src/db.js` contains branch-local duplicate declaration drift (`REQUEST_TERMINAL_STATUSES` declared twice), so local parse errors were not used as submission evidence.

## Iteration Updates (Loop 22, Iteration 17 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 17: overlap preflight (`loop-requests`, `status`, `check-completion`, and `request-history`) again prevented duplicate scope while loop-owned requests remained non-terminal (`req-ab095881`, `req-07dc7660`, `req-034d2f1d`).
- Loop 22 iteration 17: validating candidate defects against both worktree and live project-tree source plus an isolated temp-DB repro kept evidence quality high without mutating runtime state.

### Failed Patterns
- Loop 22 iteration 17: no loop-owned requests reached `completed` or `failed`, so outcome-based wording calibration remains unavailable.

### Codebase Gaps
- `coordinator/src/watchdog.js` still has an `allMerged` stale-integration completion path that finalizes requests without checking task-success consensus; isolated repro showed `integrating -> completed` with one `completed` task, one `failed` task, and a single merged PR row.
- Candidate submission is currently overlap-gated by active orchestration request `req-b78f6d2b` task #68 (`Enforce state-machine invariants and reconciliation for request-task-merge lifecycle`).

### False Positives
- Did not treat branch-local parser drift in this worktree (`coordinator/src/db.js` duplicate constant declarations) as request evidence; live project tree source was used for runtime-facing contradiction checks.
- No new request submitted this iteration because the strongest validated candidate likely overlaps active request scope.

## Iteration Updates (Loop 22, Iteration 18 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 18: overlap preflight using `loop-requests`, `status`, `check-completion`, and request-history kept submission scope bounded while loop-owned requests remained non-terminal.
- Loop 22 iteration 18: live DB verification with `better-sqlite3` on `.codex/state/codex10.db` provided concrete event evidence (`research_batch*` absent, merge-failure telemetry present) without mutating coordinator state.

### Failed Patterns
- Loop 22 iteration 18: no loop-owned requests reached `completed` or `failed`, so outcome-calibration signals remain unavailable.

### Codebase Gaps
- Runtime still accumulates merge conflicts dominated by dirty-worktree rebase failures (`git rebase origin/main` with unstaged/local changes), but this scope remains covered by active pending requests (`req-ab095881`, `req-07dc7660`) and active task work (`#80`, `#81`).
- `research_batch*` handling remains a potential guidance/code parity gap, but there is still no live runtime evidence (`mail` and `activity_log` counts are zero).

### False Positives
- Did not submit a new request this iteration because strongest candidates overlap active pending request scopes and lacked new non-overlapping contradictions.

## Iteration Updates (Loop 22, Iteration 19 - 2026-03-16)

### Successful Patterns
- Loop 22 iteration 19: checkpoint-directed overlap preflight (`loop-requests`, `status`, `check-completion`, `request-history`) again prevented duplicate submissions while loop-owned requests remained non-terminal (`req-ab095881` pending, `req-07dc7660` pending, `req-034d2f1d` decomposed).
- Loop 22 iteration 19: live source verification across `coordinator/src/watchdog.js` and `coordinator/src/merger.js` plus runtime mailbox/activity sampling on `.codex/state/codex10.db` kept evidence quality high for a no-submit decision.

### Failed Patterns
- Loop 22 iteration 19: no loop-owned requests reached `completed` or `failed`, so outcome-based wording calibration is still unavailable.

### Codebase Gaps
- `coordinator/src/watchdog.js` still contains an `allMerged` stale-integration recovery path that marks requests `completed` without validating task-success consensus (`db.checkRequestCompletion`), but this remains overlap-gated by active request `req-b78f6d2b` task #68.

### False Positives
- A suspected allocator `research_batch*` event-handling gap remains unproven this cycle: live DB checks still show zero `research_*` mail/activity rows.
- `tasks_available` high counts in mailbox telemetry were not treated as a defect because allocator-side dedupe already exists (`NOTIFY_DEDUP_MS=10000` in `coordinator/src/allocator.js`) and current evidence does not show functional breakage.

## Iteration Updates (2026-03-16, loop 22, iteration 20)

### Successful Patterns
- Loop 22 iteration 20: checkpoint-directed overlap preflight (`status`, `check-completion`, `request-history`) plus direct task-scope verification (`tasks.id=68`) prevented duplicate submission when a real watchdog/merger lifecycle defect was already covered by active request `req-b78f6d2b`.

### Codebase Gaps
- Runtime root source still shows a completion-gate gap in `coordinator/src/watchdog.js`: `recoverStaleIntegrations` marks requests `completed` on `allMerged` without checking sibling task terminal/success state, which can diverge request state from task lifecycle if integrating requests retain active tasks.
- The same lifecycle-safety theme is already actively owned by task `#68` (`req-b78f6d2b`) with explicit scope over `coordinator/src/merger.js` and `coordinator/src/watchdog.js`; defer new submissions until this lands or fails.

### False Positives
- Worktree-local source in `.worktrees/wt-4` currently includes in-flight merge-conflict drift (for example duplicate `REQUEST_TERMINAL_STATUSES` declaration in `coordinator/src/db.js`) that triggers parse errors in isolated local repros; treat this as branch-local noise and validate defects against runtime root (`/mnt/c/Users/Owner/Desktop/setup-agents-codex10`) before submitting.

## Iteration Updates (2026-03-17, loop 22, iteration 21)

### Successful Patterns
- Loop 22 iteration 21: running overlap preflight first (`status`, targeted `check-completion`, and pending `request-history`) prevented merge-conflict/docs-parity duplication and opened a distinct allocator-loop liveness submission backed by direct runtime DB telemetry plus source-path contradiction evidence (`req-a959ade0`).

### Failed Patterns
- Loop 22 iteration 21: no loop-owned requests reached `completed` or `failed`, so outcome-calibration signals remain unchanged.

### Codebase Gaps
- Fresh loop heartbeat coverage gap (2026-03-17, loop 22 iteration 21): both sentinel scripts (`.codex/scripts/loop-sentinel.sh`, `scripts/loop-sentinel.sh`) emit heartbeats during sleep/backoff windows but not during active `codex exec`, while watchdog stale threshold remains 300s in `coordinator/src/watchdog.js`. Runtime evidence from `.codex/state/codex10.db` shows loop 22 stayed `active` with `last_heartbeat=2026-03-17T01:03:40.733Z` and generated 285 `loop_heartbeat_stale` events in the last 2 hours (latest stale_sec 627), indicating healthy long iterations can be misclassified as stale.

## Iteration Updates (2026-03-17, loop 22, iteration 22)

### Successful Patterns
- Loop 22 iteration 22: checkpoint-directed overlap preflight (`loop-requests`, `status`, `check-completion`, `request-history`) again prevented duplicate submissions by confirming loop-owned requests remained non-terminal (`req-a959ade0`, `req-ab095881`, `req-07dc7660`, `req-034d2f1d`).
- Loop 22 iteration 22: validating runtime evidence against the live DB with `better-sqlite3` loaded from root `coordinator/node_modules` (fallback when local worktree lacks module installs) preserved evidence quality without mutating coordinator state.
- Loop 22 iteration 22: every-3rd-iteration memory check used the prior checkpoint `EXPLORED` field as the source of truth before deeper scans, which helped avoid context drift.

### Failed Patterns
- Loop 22 iteration 22: no loop-owned requests reached `completed` or `failed`, so there is still no fresh outcome signal to recalibrate request wording from execution results.

### Codebase Gaps
- Runtime still shows long-lived `assigned` workers with stale heartbeats (worker ids 1-4 at ~10k seconds since last heartbeat) while watchdog heartbeat escalation only evaluates `running`/`busy`; this may leave assigned-task stalls unrecovered when tmux panes remain alive.
- This candidate likely overlaps active T3 request `req-b78f6d2b` (worker liveness hardening + state-machine invariants, task #68), so it is currently tracked as overlap-gated rather than submitted.

### False Positives
- Did not submit a new request this iteration: strongest validated allocator/watchdog liveness contradiction is likely already in active scope (`req-b78f6d2b`), and merge/doc parity scopes remain covered by pending requests (`req-ab095881`, `req-07dc7660`).
- Did not use worktree-local parser drift (duplicate `REQUEST_TERMINAL_STATUSES` in `.worktrees/wt-4/coordinator/src/db.js`) as submission evidence because it is branch-local conflict noise, not runtime-root behavior.

## Iteration Updates (2026-03-16, loop 22, iteration 23)

### Successful Patterns
- Loop 22 iteration 23: checkpoint-directed overlap preflight (`status`, `check-completion`, `request-history`) before source dives prevented duplicate submissions while loop-owned requests remained non-terminal (`req-a959ade0`, `req-ab095881`, `req-07dc7660`, `req-034d2f1d`).
- Loop 22 iteration 23: direct runtime telemetry sampling from `.codex/state/codex10.db` (via `better-sqlite3`) confirmed allocator mail mix (`tasks_available`, `task_completed`, `merge_failed`, `functional_conflict`) and avoided speculative request drafting.

### Failed Patterns
- Loop 22 iteration 23: no loop-owned requests reached terminal `completed`/`failed`, so outcome-based calibration signals are still unavailable.

### Codebase Gaps
- Allocator/runtime still shows repeated merge-rebase conflict telemetry (`git rebase origin/main` dirty-worktree failures) and stale assigned-worker pressure, but both are currently overlap-gated by active requests/tasks (`req-ab095881`, `req-b78f6d2b`, task #68).
- Allocator guidance/runtime parity remains inconsistent around mailbox semantics (`merge_failed` deprecated text versus live runtime emission), and this scope is already covered by pending `req-07dc7660`.

### False Positives
- Did not submit a new request this iteration because validated candidates overlapped active requests or lacked fresh non-overlapping runtime contradictions.
- Did not promote allocator `research_batch*` guidance mismatch as a request: live DB evidence still shows zero allocator `research_batch*` mail/activity rows in this runtime snapshot.

## Iteration Updates (2026-03-17, loop 22, iteration 24)

### Successful Patterns
- Loop 22 iteration 24: checkpoint-directed overlap preflight (`loop-requests`, `status`, targeted `check-completion`, and `request-history`) prevented duplicate submissions while active loop-owned scopes remained non-terminal (`req-a959ade0`, `req-ab095881`, `req-07dc7660`, `req-034d2f1d`).
- Loop 22 iteration 24: direct source scans in `coordinator/src/allocator.js`, `coordinator/src/watchdog.js`, and `coordinator/src/cli-server.js` plus runtime status evidence kept no-submit decisions grounded in verified overlap rather than speculation.

### Failed Patterns
- Loop 22 iteration 24: no loop-owned requests reached `completed` or `failed`, so outcome-based calibration remains unavailable.

### Codebase Gaps
- Runtime still shows heavy merge-conflict pressure (`git rebase origin/main` dirty-worktree failures in merge queue) and stale assigned-worker pressure, but these remain overlap-gated by active requests (`req-ab095881`, `req-b78f6d2b`) and current task execution (`#80`, `#81`, `#68`).

### False Positives
- Did not submit a new request this iteration because the strongest validated candidates (merge conflict handling, liveness/stale assignment recovery, allocator guidance parity) overlap active pending/decomposed requests and had no distinct non-overlapping contradiction.

## Iteration Updates (2026-03-17, loop 22, iteration 25)

### Successful Patterns
- Loop 22 iteration 25: checkpoint-directed overlap preflight (`loop-requests`, `status`, `check-completion`, and `request-history`) confirmed loop-owned requests remain non-terminal and avoided duplicate submissions.
- Loop 22 iteration 25: every-3rd-iteration memory check relied on prior checkpoint `EXPLORED` scope before deeper scans, then validated runtime-root source (`coordinator/src/allocator.js`, `coordinator/src/watchdog.js`, `coordinator/src/cli-server.js`) to avoid branch-local false positives.

### Failed Patterns
- Loop 22 iteration 25: no loop-owned requests reached terminal `completed`/`failed`, so outcome-based request-shape calibration is still unavailable.

### Codebase Gaps
- Runtime still shows stale assigned workers and merge-conflict pressure, but these remain overlap-gated by active orchestration hardening (`req-b78f6d2b` task `#68`) and active merge-conflict remediations (`req-ab095881`, `req-59bfc6fb` task `#81`).

### False Positives
- Did not submit a new request this iteration because validated contradictions were already covered by active pending/decomposed requests and lacked a distinct non-overlapping scope.

## Iteration Updates (2026-03-17, loop 22, iteration 26)

### Successful Patterns
- Loop 22 iteration 26: checkpoint-directed overlap preflight (`loop-requests`, `status`, targeted `check-completion`, and `request-history`) prevented duplicate submissions while loop-owned requests stayed non-terminal (`req-a959ade0`, `req-07dc7660`, `req-ab095881`, `req-034d2f1d`).
- Loop 22 iteration 26: validating runtime-root source (`coordinator/src/allocator.js`, `coordinator/src/watchdog.js`, `coordinator/src/cli-server.js`) against live DB state (`loops` heartbeat rows, request/task/merge status) kept the no-submit decision evidence-based.

### Failed Patterns
- Loop 22 iteration 26: no loop-owned requests reached terminal `completed`/`failed`, so outcome-based request-shape calibration is still unavailable.

### Codebase Gaps
- `coordinator/src/watchdog.js` still includes stale-integration `all_merged` completion recovery that does not explicitly require task-success consensus in this worktree view, but this remains overlap-gated by active orchestration hardening request `req-b78f6d2b` task `#68`.
- Merge-conflict pressure remains high (`git rebase origin/main` with dirty worktree), but this is already covered by pending requests `req-ab095881` and `req-59bfc6fb` task `#81`.

### False Positives
- Did not submit a non-tmux loop-monitoring request: live `loops` rows currently all include `tmux_window`, so there is no fresh runtime evidence for that path in this iteration.
- Did not submit mailbox-contract parity issues (`merge_failed` guidance mismatch) because scope remains active under pending request `req-07dc7660`.

## Iteration Updates (2026-03-17, loop 22, iteration 27)

### Successful Patterns
- Loop 22 iteration 27: checkpoint-directed overlap preflight (`status`, `check-completion`, and `request-history`) again prevented duplicate submissions by confirming loop-owned scopes remain active (`req-a959ade0`, `req-07dc7660`, `req-ab095881`, `req-034d2f1d`).
- Loop 22 iteration 27: verifying task-82 scope directly in `coordinator/bin/mac10` plus the codex wrapper (`.codex/scripts/codex10`) kept the no-submit decision grounded in file-level evidence.

### Failed Patterns
- Loop 22 iteration 27: no loop-owned requests reached terminal `completed`/`failed`, so outcome-based wording calibration remains unavailable.

### Codebase Gaps
- `coordinator/bin/mac10` still has many error branches that print RPC errors without non-zero exit status, but this remains in active scope under `req-034d2f1d` task `#82`.

### False Positives
- Did not submit a new request this iteration because the strongest validated allocator-automation contradiction (CLI failure exit semantics) overlaps active decomposed work (`req-034d2f1d`) and no distinct non-overlapping defect was found.

## Iteration Updates (2026-03-17, loop 22, iteration 28)

### Successful Patterns
- Loop 22 iteration 28: checkpoint-driven overlap preflight (`loop-requests`, `status`, targeted `check-completion`) again avoided duplicate submissions while loop-owned requests remained non-terminal (`req-a959ade0`, `req-07dc7660`, `req-ab095881`, `req-034d2f1d`).
- Loop 22 iteration 28: validating runtime observations against `origin/main` source before drafting prevented a false-positive submission on failed-request remediation reopen behavior.

### Failed Patterns
- Loop 22 iteration 28: no loop-owned requests reached terminal `completed`/`failed`, so outcome-calibration signals are still unavailable for request wording.

### Codebase Gaps
- Runtime branch `/mnt/c/Users/Owner/Desktop/setup-agents-codex10` currently shows failed requests with active remediation tasks still marked `failed` (example: `req-64dd6533` with assigned task `#80`), but `origin/main` already contains `recoverFailedRequestsWithActiveRemediation` and `request_reopened_for_active_remediation` handling in `coordinator/src/watchdog.js`.
- Active overlap remains: dirty-worktree merge conflict handling (`req-ab095881`, `req-59bfc6fb` tasks `#80/#81`) and CLI failure exit semantics (`req-034d2f1d` task `#82`).

### False Positives
- Did not submit a reopen-remediation watchdog request because origin parity showed this is runtime branch drift (present in `origin/main`, absent in current runtime branch), not a new upstream defect.
- Did not submit additional merge-failure handling requests because active pending scope already covers those areas (`req-07dc7660`, `req-ab095881`).

## Iteration Updates (2026-03-17, loop 22, iteration 29)

### Successful Patterns
- Loop 22 iteration 29: running checkpoint-directed overlap preflight first (`loop-requests`, `status`, targeted `check-completion`, and `request-history`) confirmed active overlap and prevented duplicate submissions.
- Loop 22 iteration 29: reading pending request bodies before research kept scope isolation clear (`req-a959ade0`, `req-07dc7660`, `req-ab095881`, `req-034d2f1d`) and avoided re-submitting known defects.

### Failed Patterns
- Loop 22 iteration 29: no loop-owned requests reached terminal `completed`/`failed`, so there are still no fresh outcome signals for wording calibration.

### Codebase Gaps
- Active runtime still shows merge-conflict churn (`git rebase origin/main` dirty-worktree failures, including request `req-59bfc6fb` task `#81`) plus pending CLI exit-code hardening (`task #82`), but both are already covered by active pending/decomposed requests.

### False Positives
- Did not submit a new request this iteration because all strong allocator/watchdog/merger candidates overlapped active loop-owned requests, and no distinct non-overlapping defect met confidence >=0.85.

## Iteration Updates (2026-03-17, loop 22, iteration 30)

### Successful Patterns
- Loop 22 iteration 30: overlap preflight (`loop-requests`, `status`, targeted `check-completion`) again prevented duplicate submissions by confirming all loop-owned requests stayed non-terminal (`req-a959ade0`, `req-07dc7660`, `req-ab095881`, `req-034d2f1d`).
- Loop 22 iteration 30: combining source inspection (`coordinator/src/allocator.js`, `coordinator/src/watchdog.js`, `coordinator/src/index.js`) with live DB loop-state verification avoided speculative re-submission of the previously failed non-tmux stale-loop scope.

### Failed Patterns
- Loop 22 iteration 30: no loop-owned requests reached terminal `completed`/`failed`, so outcome-based request-shape calibration is still unavailable.

### Codebase Gaps
- Merge-conflict recovery pressure remains active (`req-59bfc6fb` task `#81`, `req-ab095881`) and CLI failure exit semantics remain active (`req-034d2f1d` task `#82`), leaving little non-overlapping allocator-routing surface for high-confidence new requests this cycle.
- Cross-platform loop monitoring still has a structural gap (`monitorLoops` skips rows without `tmux_window`), but this lacked current active-runtime evidence in this iteration because all active loops had tmux windows.

### False Positives
- Did not submit a new non-tmux stale-loop request this cycle: although the source contradiction remains, live loop rows currently show active loop `22` with `tmux_window` present, so fresh runtime evidence was insufficient for a confident non-overlapping request.

## Iteration Updates (2026-03-17, loop 22, iteration 31)

### Successful Patterns
- Loop 22 iteration 31: checkpoint-directed overlap preflight (`loop-requests`, `status`, targeted `check-completion`, and `request-history`) again prevented duplicate submissions by confirming loop-owned scopes remain non-terminal (`req-a959ade0`, `req-07dc7660`, `req-ab095881`, `req-034d2f1d`).
- Loop 22 iteration 31: runtime-root verification via source scans (`coordinator/src/allocator.js`, `coordinator/src/watchdog.js`) plus live DB sampling (`loops`, allocator mail type counts) kept no-submit decisions evidence-based instead of speculative.

### Failed Patterns
- Loop 22 iteration 31: no loop-owned requests reached terminal `completed`/`failed`, so outcome-based wording calibration is still unavailable.

### Codebase Gaps
- Merge-conflict remediation and merge-failure guidance parity remain active but overlap-gated by pending requests (`req-ab095881`, `req-07dc7660`) and active task execution (`req-59bfc6fb` task `#81`).
- Worker/lifecycle hardening candidates in watchdog remain overlap-gated by active orchestration request `req-b78f6d2b`.

### False Positives
- Did not submit a non-tmux loop-monitoring request this iteration: live DB query shows the only active loop (`id=22`) currently has `tmux_window='loop-22'`, so there is no fresh runtime evidence for the no-window path.
- Did not submit allocator research-mail parity issues: live mailbox aggregate currently shows allocator mail types `tasks_available`, `task_completed`, `merge_failed`, `functional_conflict`, `tasks_ready`, `task_failed` with no `research_batches_available` rows in this runtime snapshot.

## Iteration Updates (2026-03-17, loop 22, iteration 32)

### Successful Patterns
- Loop 22 iteration 32: checkpoint-directed overlap preflight (`loop-requests`, `status`, targeted `check-completion`, and `request-history`) again prevented duplicate submissions by confirming loop-owned requests remain non-terminal (`req-a959ade0`, `req-07dc7660`, `req-ab095881`, `req-034d2f1d`).
- Loop 22 iteration 32: source inspection in `coordinator/src/allocator.js`, `coordinator/src/watchdog.js`, and `coordinator/src/cli-server.js` was paired with live DB checks for active loop rows before considering new watchdog/allocator submissions.

### Failed Patterns
- Loop 22 iteration 32: no loop-owned requests reached terminal `completed` or `failed`, so there are still no fresh outcome signals for wording calibration.

### Codebase Gaps
- Merge conflict remediation remains active and overlap-gated by pending/decomposed work (`req-ab095881`, `req-59bfc6fb` task `#81`), while CLI error-exit hardening is still overlap-gated by `req-034d2f1d` task `#82`.

### False Positives
- Did not submit a non-tmux loop-monitoring request this iteration because live DB evidence still shows the only active loop (`id=22`) has `tmux_window='loop-22'`.
- Did not submit a new allocator/watchdog request because validated contradictions were already covered by active pending/decomposed scopes and no distinct non-overlapping defect met confidence >= 0.85.

## Iteration Updates (2026-03-17, loop 22, iteration 33)

### Successful Patterns
- Loop 22 iteration 33: running overlap preflight first (`loop-requests`, `status`, targeted `check-completion`, and `request-history`) again prevented duplicate submissions by confirming loop-owned scopes remain non-terminal (`req-a959ade0`, `req-07dc7660`, `req-ab095881`, `req-034d2f1d`) while `req-59bfc6fb` is still mid-flight (`2/5 completed`).
- Loop 22 iteration 33: pairing source audits (`coordinator/src/allocator.js`, `coordinator/src/watchdog.js`, `coordinator/src/cli-server.js`) with live DB checks (active loop rows in `.codex/state/codex10.db`) kept no-submit decisions grounded in current runtime evidence.

### Failed Patterns
- Loop 22 iteration 33: no loop-owned requests reached terminal `completed`/`failed`, so outcome-based request wording calibration remains unavailable.

### Codebase Gaps
- Active overlap is still concentrated in pending/decomposed merge/remediation and CLI-exit semantics scopes (`req-ab095881`, `req-07dc7660`, `req-034d2f1d`, and active task `#81` under `req-59bfc6fb`), leaving no clear non-overlapping allocator/watchdog fix candidate this cycle.

### False Positives
- Did not submit a non-tmux watchdog request: live loop row evidence still shows the only active loop (`id=22`) has `tmux_window='loop-22'`, so no fresh runtime proof for the no-window path.
- Did not submit a watchdog completion-gating request from `recoverStaleIntegrations` this iteration because runtime queries found no current `integrating` requests and no recent `stale_integration_recovered` events with `reason=all_merged`, so impact evidence was insufficient for confidence >= 0.85.

## Iteration Updates (2026-03-17, loop 22, iteration 34)

### Successful Patterns
- Loop 22 iteration 34: checkpoint-directed overlap preflight (`loop-requests`, `status`, targeted `check-completion`) continued to prevent duplicate submissions by confirming loop-owned scopes are still non-terminal (`req-a959ade0`, `req-07dc7660`, `req-ab095881`, `req-034d2f1d`).
- Loop 22 iteration 34: pairing source inspection (`coordinator/src/watchdog.js`, `coordinator/src/allocator.js`, `coordinator/src/cli-server.js`, `coordinator/src/db.js`, `coordinator/bin/mac10`) with runtime DB evidence kept the no-submit decision evidence-driven.
- Loop 22 iteration 34: reviewing one completed (`req-ed004636`) and one failed (`req-f345a353`) request history preserved the established request-shape calibration (explicit WHAT/WHERE/WHY/EVIDENCE packets) for future submissions.

### Failed Patterns
- Loop 22 iteration 34: no loop-owned requests reached terminal `completed`/`failed` this cycle, so there is still no fresh execution-outcome feedback for wording adjustments.

### Codebase Gaps
- `check-completion` currently reports `ALL DONE` when all tasks are completed even if `request_status='failed'` (runtime example: `req-f345a353`), which can create operator ambiguity; this remains overlap-gated by active lifecycle hardening scope in `req-b78f6d2b`.
- Merge conflict remediation and CLI failure-exit hardening remain active overlap areas (`req-ab095881`, `req-07dc7660`, `req-034d2f1d`, and request `req-59bfc6fb` task `#81`).

### False Positives
- Did not submit a new request this iteration because validated contradictions were either overlap-gated by active requests/tasks or lacked a distinct non-overlapping impact path with confidence >= 0.85.
- Did not submit non-tmux loop-monitoring scope because live loop rows still show the active loop (`id=22`) with `tmux_window='loop-22'`.

## Iteration Updates (2026-03-17, loop 22, iteration 35)

### Successful Patterns
- Loop 22 iteration 35: checkpoint-directed overlap preflight (`loop-requests`, `status`, targeted `check-completion`) again prevented duplicate submissions by confirming loop-owned requests remain non-terminal (`req-a959ade0`, `req-07dc7660`, `req-ab095881`, `req-034d2f1d`) while `req-59bfc6fb` is still in-progress (`2/5 completed`).
- Loop 22 iteration 35: pairing runtime evidence (`status`, `inbox allocator`, allocator mail-type counts from `.codex/state/codex10.db`) with source review (`coordinator/src/allocator.js`, `coordinator/src/cli-server.js`, `coordinator/src/db.js`, `coordinator/bin/mac10`) kept the no-submit decision evidence-backed.

### Failed Patterns
- Loop 22 iteration 35: no loop-owned requests reached terminal `completed`/`failed`, so outcome-based request wording calibration remains unavailable.

### Codebase Gaps
- Active overlap remains concentrated in merge/conflict remediation and CLI failure semantics (`req-ab095881`, `req-07dc7660`, `req-034d2f1d`, and active task `#81` for `req-59bfc6fb`), leaving no clear non-overlapping allocator-routing defect this cycle.

### False Positives
- Did not submit allocator research-mail parity scope this iteration: allocator mailbox aggregate still shows only `tasks_available`, `task_completed`, `merge_failed`, `functional_conflict`, `tasks_ready`, and `task_failed` types in runtime data, with no `research_batches_available` evidence.
- Did not submit a new request because validated contradictions were either overlap-gated by active requests/tasks or below confidence >= 0.85 for net-new pipeline work.

## Iteration Updates (2026-03-17, loop 22, iteration 36)

### Successful Patterns
- Loop 22 iteration 36: checkpoint-directed overlap preflight (`loop-requests`, `status`, and targeted `check-completion`) again prevented duplicate submissions by confirming loop-owned requests remain non-terminal (`req-a959ade0`, `req-07dc7660`, `req-ab095881`, `req-034d2f1d`) while active routing remediation tasks (`#80`, `#81`) and CLI fix task (`#82`) remain in-flight.
- Loop 22 iteration 36: reading active request bodies (`request-history`) before deeper source audits kept scope boundaries explicit and avoided resubmitting covered merge-prep/CLI-exit/prompt-parity defects.

### Failed Patterns
- Loop 22 iteration 36: no loop-owned requests reached terminal `completed`/`failed`, so there are still no fresh loop-specific outcome signals for wording calibration.

### Codebase Gaps
- Active overlap remains concentrated in merge remediation and CLI failure-semantics hardening (`req-ab095881`, `req-07dc7660`, `req-034d2f1d`) with runtime evidence showing `#80/#81` assigned and `#82` ready.
- `coordinator/src/watchdog.js` still emits `merge_failed` mails while allocator guidance parity fixes remain pending, so no net-new allocator/watchdog request met non-overlap criteria this cycle.

### False Positives
- Did not submit a new request because strong candidates were already covered by active pending/decomposed requests and no distinct allocator/watchdog defect reached confidence >= 0.85 with fresh non-overlapping runtime evidence.
