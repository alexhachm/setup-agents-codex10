...
## 2026-03-16 — terminal->active request metadata clearing (task 71)
- Centralize request lifecycle cleanup in `db.updateRequest`: when `status` transitions from terminal (`completed`/`failed`) to any active non-terminal status, force `completed_at` and `result` to `NULL` in the same write.
- Keeping merger/CLI request status writes on `db.updateRequest(..., { status: 'integrating' })` automatically propagates cleanup across integrate/reopen/retry paths without duplicating logic in callers.
- Regression coverage should span direct DB state transitions plus merger `onTaskCompleted` and CLI `integrate`/reopen flows to prevent stale completion metadata on active requests.
