...
## 2026-03-16 — Merge queue dedupe by request + PR ownership identity
- `enqueueMerge` dedupe must use `request_id + pr_url + branch` (not `request_id + task_id`) so repeated complete-task cycles for the same request/PR ownership refresh one row.
- `queueMergeWithRecovery` should look up existing rows by the same identity and rebind `task_id` on refresh; otherwise follow-up tasks in the same request can be misclassified as ownership collisions.
- Keep cross-request PR collision guards keyed on `pr_url` with ownership mismatch conditions (`request_id`/`branch`) so different requests sharing branch names still integrate independently when PR URLs differ.

## 2026-03-16 — Non-tmux stale loop watchdog recovery
- In `watchdog.monitorLoops`, do not hard-skip active loops with `tmux_window = null`; evaluate heartbeat staleness for those loops using fallback timestamps (`last_heartbeat` first, then `updated_at`/`created_at`).
- For stale non-tmux loops, emit `loop_heartbeat_stale` and drive the same recycle path (`loop_sentinel_dead` + relaunch + `loop_sentinel_respawned`) using detached non-tmux sentinel spawning.
- Preserve tmux path behavior while adding non-tmux handling; add regression coverage for both non-tmux stale recovery and tmux stale-monitoring parity.

## 2026-03-16 — Task 78 merge-conflict checkpoint for merger files
- For merge-conflict tasks scoped to `coordinator/src/merger.js` and `coordinator/tests/merger.test.js`, first verify `git diff origin/main -- <files>` and conflict-marker scan before editing; recent relands may already be present.
- When scoped diffs are empty and tests pass, complete as validation-only and report explicit validation evidence (`cd coordinator && npm test -- tests/merger.test.js`).

## 2026-03-16 — Watchdog stale-loop cadence alignment (Task 75)
- In `monitorLoops`, derive stale-loop threshold from sentinel heartbeat cadence semantics (`30s * 12` by default, overrideable via `loop_stale_heartbeat_sec`) rather than an opaque literal.
- Include `threshold_sec` in `loop_heartbeat_stale` logs so activity-log telemetry shows both observed age and the decision boundary.
- Regression coverage should include a healthy long-backoff loop just below threshold (no stale log/respawn) plus a stale loop above threshold (stale log + forced respawn).

## 2026-03-16 — Merge-conflict reland with dirty worktree guard
- For merge-conflict reland tasks that fail on `git rebase origin/main` due unstaged workspace changes, stash with `--include-untracked`, run the mandatory fetch/rebase, then pop stash to preserve local artifacts.
- For validation-only merge checkpoints on `coordinator/src/merger.js` + `coordinator/tests/merger.test.js`, verify both no conflict markers and zero scoped diff against `origin/main` before reporting completion.

## 2026-03-16 — Task 67 bounded stalled-assignment recovery
- Added shared `db.recoverStalledAssignments` to reclaim `assigned`/`in_progress` tasks when worker liveness is stale, worker/task ownership pointers drift, or workers are idle/missing.
- Recovery is bounded by persistent `tasks.liveness_reassign_count` with configurable limit (`watchdog_task_reassign_limit`); exhausted tasks fail deterministically instead of looping reassignments.
- Both watchdog and allocator now invoke the same recovery path so stalled-assignment throughput resumes even when one loop path is delayed.
- Recovery emits structured diagnostics (`task_liveness_recovered`, `task_liveness_retry_exhausted`, `stalled_assignment_recovery`) with reason/source/staleness metadata for operators.

## 2026-03-16 — Task 79 merge-conflict validation checkpoint
- For merger merge-conflict reland tasks scoped to `coordinator/src/merger.js` and `coordinator/tests/merger.test.js`, verify `git diff origin/main -- <files>` and conflict-marker scan first; this request resolved as validation-only when both were clean after rebase.
- Tier-2 test execution in this worktree remains blocked by a pre-existing `coordinator/src/db.js` duplicate `REQUEST_TERMINAL_STATUSES` declaration syntax error; record as unrelated blocker when scoped files are unchanged.
