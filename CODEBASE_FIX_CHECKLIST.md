...
- [x] **P2: Split `coordinator/src/cli-server.js` only after the integrated branch is green and live-audited.** Command schemas/protocol, runtime health, merge queue service helpers, shared CLI test harness, and command-domain handlers were extracted while preserving public CLI behavior.
  ...
  - [x] Extract domain-analysis command execution to `coordinator/src/commands/domain-analysis.js`, leaving command schemas and socket validation in `cli-server.js`. Added focused CLI coverage for create/fetch/list/submit/approve/reject and approved domain doc writes. Focused validation: `node --check coordinator/src/cli-server.js`, `node --check coordinator/src/commands/domain-analysis.js`, `node --check coordinator/tests/cli.test.js`, `node --test --test-name-pattern="domain analysis" coordinator/tests/cli.test.js` -> 2 passing.
  - [x] Extract extended-research command execution to `coordinator/src/commands/extended-research.js`, leaving command schemas and socket validation in `cli-server.js`. Added focused CLI coverage for research topic create/fetch/list/review/pending-review flow and `fill-knowledge` research/rescan actions. Focused validation: `node --check coordinator/src/cli-server.js`, `node --check coordinator/src/commands/extended-research.js`, `node --check coordinator/tests/cli.test.js`, `node --test --test-name-pattern="extended research" coordinator/tests/cli.test.js` -> 2 passing.
  - [x] Extract research-queue command execution to `coordinator/src/commands/research-queue.js`, leaving command schemas and socket validation in `cli-server.js`. Expanded focused CLI coverage for queue/status/next/gaps/start/fail/retry, stale requeue, and completion metadata reset. Focused validation: `node --check coordinator/src/cli-server.js`, `node --check coordinator/src/commands/research-queue.js`, `node --check coordinator/tests/cli.test.js`, `node --test --test-name-pattern="research-queue" coordinator/tests/cli.test.js` -> 11 passing.
  - [x] Extract remaining command domains from the parallel worktrees: architect, allocator, browser-offload, integration, loop, system, user/status, worker completion/failure, and worker lifecycle. Final validation after command and DB integration: `cd coordinator && npm test` -> 692 passing, 0 failing; `scripts/preflight.sh` -> pass.
- [x] **P2: Split `coordinator/src/db.js` after `cli-server.js` boundaries are stable.** `db.js` is now a compatibility facade over repository modules for migrations, requests, tasks, workers, merge queue, mail, config, log, browser, memory, research, changes, loops, domain analysis, and extended research. Focused validation covered migration/state-machine/security, allocator/watchdog, merger/CLI, browser/research batching, memory indexing, and insight ingestion; final validation: `cd coordinator && npm test` -> 692 passing, 0 failing; `scripts/preflight.sh` -> pass.
- [ ] **P2: Enable non-Claude providers only after the Claude provider path survives the live audit on the integrated branch.** For each provider, require local CLI/auth health, launch dry-run, one noninteractive task smoke, and output-usage schema validation before setting `enabled: true`. Provider enablement harness is now available through `scripts/provider.sh validate [provider] [project_dir] [--runtime] [--json]`; Claude runtime validation passes, Codex CLI/version checks pass but remains disabled until a real noninteractive task smoke is approved, and Gemini/DeepSeek remain blocked because their CLIs are not on PATH.
