...
- [ ] **P0: Land the live-audit startup fixes as a checkpoint before more migration work.** Fixes made during the audit: setup same-file copy guard and CLI socket close (`929be96`), WSL namespace propagation (`0506230`), coordinator detachment via `setsid`, worker sentinel CLI path resolution, Docker worker entrypoint clearing, Dockerfile `CMD`, Docker root project mount, and `coordinator/tests/worker-backend.test.js`.
- [ ] **P0: Make default worker isolation truthful and deterministic.** `liveaudit102` defaulted to `msb` because the server was present, but task `#1` failed three times with `msb_sandbox_dead`. The coordinator must only select `msb` after a real sentinel launch health check, or fall back to Docker/tmux automatically before burning task retries.
- [ ] **P0: Block stale branch/PR reuse during worker completion and merge.** Retry task `#2` completed the request, but completion recovered stale branch `agent-1` / PR `#359` from the worker home even though the task was a no-edit smoke. Require assignment-token ownership, a task-created branch or explicit no-edit completion, and ignore duplicate `complete-task` calls after terminal completion.
- [ ] **P0: Replace model-inferred health reports with coordinator-served health context.** Tier 1 `req-5d5314b9` completed, but reported `sandbox_mode=tmux(default)` and `research_driver=not_running` while direct CLI evidence showed Docker-first before worker assignment and research item `#1` completed. Tier 1 health tasks should call a structured `mac10 health`/context endpoint instead of asking the model to infer runtime state.
- [ ] **P1: Implement disposable per-task sandbox lifecycle as its own bounded phase after landing.** First task should define DB/API state and lifecycle commands; second task should wire worker execution; third task should add cleanup policy and tests. Do not mix this with branch reconciliation.
- [ ] **P1: Make Docker worker execution provider-ready.** The Docker launch path now gets past the image entrypoint issue and can see root project state, but the image/provider contract still needs a smoke that proves the selected provider CLI exists and can run noninteractively inside the worker container.
- [ ] **P1: Implement coordinator-served context bundles before broad module splitting.** Start with one CLI/API command that returns task assignment, safe edit files, validation command, relevant domain knowledge, known pitfalls, recent related failures, and runtime health for a task ID.
