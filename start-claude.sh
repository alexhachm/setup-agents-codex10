#!/usr/bin/env bash
# start-claude.sh — Start the mac10 multi-agent system using Claude as the provider.
#
# Usage:
#   bash start-claude.sh <project_dir> [num_workers]
#   bash start-claude.sh --stop <project_dir>
#   bash start-claude.sh --pause <project_dir>
set -euo pipefail
...
if [ "${1:-}" = "--stop" ] || [ "${1:-}" = "--pause" ]; then
  ACTION="$1"
  PROJECT_DIR="${2:?Usage: bash start-claude.sh $ACTION <project_dir>}"
  PROJECT_DIR="$(cd "$PROJECT_DIR" 2>/dev/null && pwd || echo "$PROJECT_DIR")"
  NAMESPACE="${MAC10_NAMESPACE:-codex10}"
  CODEX_DIR="$PROJECT_DIR/.codex"
  CODEX10_CLI="$CODEX_DIR/scripts/codex10"
  # Source common for stop/pause functions
...
# --- Normal startup ---
PROJECT_DIR="${1:?Usage: bash start-claude.sh <project_dir> [num_workers]}"
NUM_WORKERS="${2:-4}"
MAX_WORKERS=8
NAMESPACE="${MAC10_NAMESPACE:-codex10}"
...
echo "========================================"
echo " mac10 Multi-Agent Setup (Claude)"
echo "========================================"
