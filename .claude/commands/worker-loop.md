...
4. **Send heartbeats** every 30 seconds during long work:
   ...
   ```
5. **Self-verify**: run only explicit task-provided validation commands
   - If validation shows shorthand metadata (`tier2`/`tier3`), treat it as workflow metadata, not a shell command.
   - Never infer an implicit `npm run build`.
