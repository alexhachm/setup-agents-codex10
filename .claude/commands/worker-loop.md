...
4. **Send heartbeats** every 30 seconds during long work:
   ...
   ```
5. **Self-verify**: run only the task-provided validation commands (or assigned validator flow); do not assume `npm run build`
...
- Treat tier shorthand values (for example `tier2`/`tier3`) as routing hints, not shell commands
...
Use `[result]` for a short completion summary when available.
