...
4. **Send heartbeats** every 30 seconds during long work:
   ...
   ```
5. **Self-verify**: run only explicit task-provided validation commands. If validation is shorthand metadata (`tier2`/`tier3`), do not execute it as a command and do not infer implicit `npm run build`.
