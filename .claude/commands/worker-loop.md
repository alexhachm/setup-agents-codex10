...
4. **Send heartbeats** every 30 seconds during long work:
   ...
   ```
5. **Self-verify**:
   - Run only explicit task-provided validation commands.
   - Treat shorthand `validation` values (`tier2`/`tier3`) as workflow metadata, not shell commands.
   - Never infer implicit commands like `npm run build`.
