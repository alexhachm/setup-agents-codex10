...
| Check your inbox for requests | `mac10 inbox architect` |
| Wait for requests | `mac10 inbox architect --block` |
...
| Show request details | `mac10 show <request_id>` |
| Get task details | `mac10 get-task <task_id>` |
| List/filter tasks | `mac10 list-tasks [--request X] [--status Y]` |
...
- YOU claim an idle worker via `mac10 claim-worker`, create task via `mac10 create-task`, assign via `mac10 assign-task`.
...
6. Exit and relaunch using the `/scan-codebase` prompt template
