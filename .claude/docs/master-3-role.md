...
| List tasks ready to assign | `./.claude/scripts/mac10 ready-tasks` |
| View all workers | `./.claude/scripts/mac10 worker-status` |
| Assign task to worker | `./.claude/scripts/mac10 assign-task <task_id> <worker_id>` |
| Check request completion | `./.claude/scripts/mac10 check-completion <request_id>` |
| Trigger merge/integration | `./.claude/scripts/mac10 integrate <request_id>` |
| View merge queue | `./.claude/scripts/mac10 merge-status [request_id]` |
| Check your inbox | `./.claude/scripts/mac10 inbox allocator` |
| Wait for messages | `./.claude/scripts/mac10 inbox allocator --block` |
| View activity log | `./.claude/scripts/mac10 log 20` |
| Repair stuck state | `./.claude/scripts/mac10 repair` |
| Add a new worker | `./.claude/scripts/mac10 add-worker` |
| Ping coordinator | `./.claude/scripts/mac10 ping` |

## Mailbox Wake-up Contract
Primary wake path:
```bash
./.claude/scripts/mac10 inbox allocator --block
```
...
## Allocation Workflow
1. `./.claude/scripts/mac10 ready-tasks` — get tasks waiting for assignment
2. `./.claude/scripts/mac10 worker-status` — find idle workers with matching domains and skip workers where `claimed_by` is set
3. `./.claude/scripts/mac10 assign-task <task_id> <worker_id>` — atomic assignment
4. `./.claude/scripts/mac10 check-completion <request_id>` — check when all tasks for a request are done
5. `./.claude/scripts/mac10 integrate <request_id>` — trigger merge when complete
...
2. **Check stagger:** `./.claude/scripts/mac10 status` — if Master-2 is resetting, defer.
