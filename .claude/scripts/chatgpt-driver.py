...
# Support root scripts/ and legacy nested script locations.
...
PROFILE_DIR = Path.home() / f".chatgpt-mac10-profile-{_project_hash}"
HEALTH_FILE = PROJECT_DIR / ".claude" / "state" / "agent-health.json"
LOG_FILE = PROJECT_DIR / ".claude" / "logs" / "research-driver.log"
MAC10_CMD = PROJECT_DIR / ".claude" / "scripts" / "mac10"
LOCK_FILE = PROJECT_DIR / ".claude" / "state" / "research-driver.lock"
...
def run_mac10(*args):
    """Run a mac10 CLI command and return output."""
    cmd = [str(MAC10_CMD)] + list(args)
    try:
        result = subprocess.run(cmd, capture_output=True, text=True, timeout=15)
        return result.stdout.strip(), result.returncode
    except Exception as e:
        log.warning(f"mac10 command failed: {e}")
        return "", 1


def get_next_queued():
    """Get the next queued research item from the coordinator."""
    output, code = run_mac10("research-next")
...
def mark_in_progress(item_id):
    ...
    _, code = run_mac10("research-start", str(item_id))
    if code == 0:
...
class ResponseDetector:
    ...
    def _handle_streaming(self, s):
        ...
        Uses text-staleness detection: if the assistant's text hasn't changed
        in `stream_stale_timeout` seconds, the stream is likely done even if
        CSS streaming indicators are still present (common with Extended
        Thinking where the indicator lingers after generation completes).
        The hard `streaming_timeout` remains as an absolute safety cap.
        """
        # Standard modes: check streaming indicators + text staleness
        if self.mode != "deep_research":
            stream_stale = self.config.get("stream_stale_timeout", 20.0)
            stream_timeout = self.config.get("streaming_timeout", 600)

            # Check text staleness via JS-side timestamp
            last_changed = s.get("lastAssistantTextChangedAt", 0)
            seconds_since_change = (time.time() * 1000 - last_changed) / 1000.0

            if self._is_streaming(s):
                # Text hasn't changed in stream_stale_timeout — stream is done
                if seconds_since_change >= stream_stale:
                    # Only if we have some content (not just the thinking preamble starting)
                    if s.get("assistantMessageCount", 0) > self.initial_msg_count:
                        log.info(
                            f"ResponseDetector: text stale for {seconds_since_change:.1f}s "
                            f"while streaming — treating as complete"
                        )
...
class TabPool:
    ...
    async def _dispatcher_loop(self):
        ...
        while not self._shutdown:
            ...
            if slot.state != TabSlotState.IDLE:
                ...
                run_mac10("research-fail", str(item_id), "Tab open failed")
                if slot in self.slots:
                    self.slots.remove(slot)
                continue
            ...
            try:
                ...
                run_mac10("research-fail", str(item_id), str(e))
    ...
    async def _collect_lifecycle(self, slot):
        ...
        try:
            ...
            if note_path:
                run_mac10("research-complete", str(item_id), note_path)
                log.info(f"Slot {slot.slot_id}: item #{item_id} completed → {note_path}")
            else:
                run_mac10("research-fail", str(item_id), "Ingestion failed")
...
async def run_test_suite(bm, page):
    ...
    test_output = PROJECT_DIR / ".claude" / "logs" / "test-results.json"
...
async def main_loop():
    ...
    output, _rc = run_mac10("research-requeue-stale", "0")
    if output:
