...
3. **Validate** using explicit task-provided commands (no implicit `npm run build`)
...
5. **Validate.** Tier 2: build-validator. Tier 3: build-validator + verify-app. Treat `tier2`/`tier3` in task metadata as workflow shorthand, not shell commands.
