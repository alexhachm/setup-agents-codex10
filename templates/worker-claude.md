...
3. **Validate** your work using explicit task-provided commands (no implicit `npm run build`)
...
5. **Validate.** Tier 2: build-validator. Tier 3: build-validator + verify-app. `tier2`/`tier3` are workflow metadata, so run only explicit task commands.
