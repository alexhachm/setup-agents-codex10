...
3. **Validate** your work using explicit task-provided commands only (build/test/lint as specified)
...
5. **Validate.** Tier 2: build-validator. Tier 3: build-validator + verify-app. Treat `tier2`/`tier3` as workflow metadata and never infer implicit `npm run build`.
