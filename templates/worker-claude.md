...
5. **Validate.** Tier 2: build-validator. Tier 3: build-validator + verify-app. Treat `tier2`/`tier3` labels as metadata and run only explicit task validation commands (no implicit `npm run build`).
