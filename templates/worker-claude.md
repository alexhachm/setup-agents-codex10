...
3. **Validate** using explicit task-provided commands
...
5. **Validate.** Tier 2: build-validator. Tier 3: build-validator + verify-app. For `tier2`/`tier3` validation labels, treat them as metadata and run only explicit task commands (no implicit `npm run build`).
