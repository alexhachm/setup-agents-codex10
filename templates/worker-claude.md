...
3. **Validate** your work using task-provided build/test/lint commands
...
5. **Validate.** Tier 2: build-validator. Tier 3: build-validator + verify-app. Treat `tier2`/`tier3` as workflow metadata and run only explicit task validation commands (never assume `npm run build`).
