...
4. **Send heartbeats** every 30 seconds during long work:
   ...
   ```
5. **Self-verify**: run only the explicit build/test/lint commands from the task's validation field. Treat `tier2`/`tier3` as metadata (not shell commands), and do not assume `npm run build`.
