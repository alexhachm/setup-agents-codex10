...
4. **Send heartbeats** every 30 seconds during long work:
   ...
   ```
5. **Self-verify**: run only explicit task-provided validation commands. Treat `tier2`/`tier3` as workflow metadata (not shell commands), and do not infer an implicit `npm run build`.
...
For validation-only/no-PR completions, pass only the result summary after `task_id`.
