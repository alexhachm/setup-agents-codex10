...
4. **Send heartbeats** every 30 seconds during long work:
   ...
   ```
5. **Self-verify**: run only explicit commands provided by the task validation metadata/description; treat `tier2`/`tier3` as workflow labels (not shell commands) and never assume implicit `npm run build`
