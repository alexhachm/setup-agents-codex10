...
4. **Send heartbeats** every 30 seconds during long work:
   ...
   ```
5. **Self-verify**: run only explicit build/test/lint commands provided in the task payload. If validation is shorthand like `tier2`/`tier3`, treat it as metadata only and do not infer commands such as `npm run build`.
